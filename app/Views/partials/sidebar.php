<aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="<?= url('/aprovacoes') ?>" class="nav-item <?= is_active_nav('/aprovacoes') ?>">
            <span class="nav-icon">✓</span><span class="nav-label">Aprovações</span>
        </a>
        <?php endif; ?>
        <?php if (\App\Core\Auth::can('customers.manage')): ?>
        <a href="<?= url('/clientes') ?>" class="nav-item <?= is_active_nav('/clientes') ?>">
            <span class="nav-icon">👥</span><span class="nav-label">Clientes</span>
        </a>
        <?php endif; ?>
        <?php if (\App\Core\Auth::can('sales.create')): ?>
        <a href="<?= url('/vendas/nova') ?>" class="nav-item <?= is_active_nav('/vendas/nova') ?>">
            <span class="nav-icon">🛒</span><span class="nav-label">Nova Venda</span>
        </a>
        <a href="<?= url('/vendas') ?>" class="nav-item <?= is_active_nav('/vendas') ?>">
            <span class="nav-icon">🧾</span><span class="nav-label">Vendas</span>
        </a>
        <?php endif; ?>
    </nav>
